...
void griphein(buffer_t *bp, char_t *p)
{
	if ((ptr(bp, bp->b_mark) == p) && (bp->b_mark != NOMARK)) {
		addch(*p | A_REVERSE);
	} else if (pos(bp,p) == bp->b_point && bp->b_paren != NOPAREN) {
...
void display(window_t *wp, int flag)
{
	...
	buffer_t *bp = wp->w_bufp;
        int inwrd= 0; /*EdMort*/
	setLanguage(getExtension(bp->b_fname)); /*EdMort*/
	...
	while (1) {
		...
		if (*p != '\r') {
			...
			if ( nch > 1) {
				...
				j += *p == '\t' ? 8-(j&7) : 1;
			     if (inwrd > 0) {griphein(bp, p); inwrd= inwrd-1;}
			     else { inwrd= kwrd(p);
			       if (inwrd > 0)
				 { griphein(bp, p); inwrd= inwrd - 1;}
			       else {display_char(bp, p);}}
